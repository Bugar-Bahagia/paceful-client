    ...
    try {
      ...
      setData((prevData: Goal[]) => {
        if (reset === true) {
          ...
          const newGoals = goals.filter((goal: Goal) => !prevData.some((item: Goal) => item.id === goal.id));
  ...
  return (
    <div className="bg-white  min-h-screen">
      <header className="text-center pt-10">
        <h1 className="text-4xl font-bold text-black">ACHIEVE YOUR GOALS, STEP BY STEP</h1>
        <p className="mt-4 text-lg text-black">"Every small step you take brings you closer to your bigger dreams. Keep going, don’t stop now, and prove to yourself that you can finish what you started."</p>
      </header>

      <div className="flex justify-center items-center pt-10">
        <button style={{ color: '#ffffff' }} className="bg-orange-600 text-black px-6 py-2 rounded-lg hover:bg-green-600 focus:outline-none" onClick={() => openModal()}>
          Add New Goal
        </button>
      </div>
          ...
          data.map((goal) => (
            ...
            >
              <div className="card-body flex flex-col items-center text-center">
                <h3 className="card-title text-xl font-bold text-black-400 underline ">{goal.typeName.toUpperCase()}</h3>

                {/* Progress Status */}
                <div className="">
                  <p className="text-xl font-semibold text-center pb-1 ">{goal.isAchieved ? 'Achieved' : 'On Progress'}</p>
                  <p className="text-lg font-semibold">{goal.isAchieved ? 100 : Math.round((goal.currentValue / goal.targetValue) * 100)}%</p>
                </div>
                <span>---------------------------</span>
                {/* Goal Information */}
                ...
                <div className="card-actions flex gap-4 justify-center mt-4">
                  ...
                  <button onClick={() => openModal(goal.id)} className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">
                    Update
                  </button>
                </div>
              </div>
        ...
        <div className="modal-box bg-white p-6 rounded-lg shadow-lg">
          ...
          {selectedGoalId ? (
            <>
              ...
              <UpdateGoal setGoal={setData} setPage={setPage} goalId={selectedGoalId} onGoalUpdated={fetchGoals} />
            </>
          ) : (
            <>
              <h3 className="font-bold text-lg text-teal-400">Create New Goal</h3>
              <CreateGoal fetchGoals={fetchGoals} setPage={setPage} />
            </>
          )}
        </div>
      </dialog>
